...
public class RegexUtil {
     ...
     * @return
     */
    public static String subDoubleFromStr(String str) {
        String s = "";
        if (StringUtils.isEmpty(str)) {
            return s;
        }
     ...
     * @return
     */
    public static int getLastCharIndex(String str) {
        if (StringUtils.isEmpty(str)) {
            return -1;
        }
        int m = 0;
    ...
    public static Double _evalPrice(String value, HashMap<String, Object> subs) {
        ScriptEngineManager manager = new ScriptEngineManager();
        ScriptEngine engine = manager.getEngineByName("js");
        try {
            subs.put("ceil", "Math.ceil");
            subs.put("floor", "Math.floor");
            for (Iterator<String> iterator = subs.keySet().iterator(); iterator.hasNext();) {
                String subKey = iterator.next();
                value = value.replaceAll(subKey, subs.get(subKey).toString());
            }
