...
qiao.bs.modalstr = function(opt){
	var start = '<div class="modal '+opt.fade+'" id="' + opt.id + '" tabindex="-1" role="dialog" aria-labelledby="bsmodaltitle" aria-hidden="true" style="position:fixed;top:0px;'+opt.style+'">';
	if(opt.big){
	...
	var body = '<div class="modal-body" style="max-height:580px;overflow-y:auto;"><p><h5>'+opt.msg+'</h5></p></div>';
	
	var foot = '';
	if(opt.foot){
		foot += '<div class="modal-footer">';
		if(opt.btn){
			foot += '<button type="button" class="btn btn-default bscancel btn-sm">'+opt.qubtn+'</button>';
		}
		foot += '<button type="button" class="btn btn-primary bsok btn-sm">'+opt.okbtn+'</button>';
		foot += '</div>';
	}
...
qiao.bs.dialog = function(options, func){
	...
	if(opt.url != ''){
    	...
    	$('#' + opt.id + ' div.modal-body').empty().append(opt.html);
		if(options.callback) options.callback();
    } 
