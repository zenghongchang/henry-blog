...
import com.tofba.blog.model.enums.*;
...
import com.tofba.blog.utils.SensitiveFilter;
...
public class FrontCommentController {
    ...
    @ResponseBody
    public List<Comment> loadComment(@RequestParam(value = "page") Integer page, @RequestParam(value = "post") Post post) {
        Sort sort = new Sort(Sort.Direction.DESC, "commentDate");
    ...
    @ResponseBody
    public JsonResult newComment(@Valid @ModelAttribute("comment") Comment comment, BindingResult result, @ModelAttribute("post") Post post, HttpServletRequest request) {
        if (result.hasErrors()) {
        ...
        try {
            ...
            char replace = '*';
            String newCommentContent = new SensitiveFilter().filter(commentContent, replace);
            if (!Objects.equals(commentContent, newCommentContent)) {
                return new JsonResult(ResultCodeEnum.FAIL.getCode(), "评论失败！留言内容包含敏感词汇，请文明留言");
            }
    ...
    class EmailToAdmin extends Thread {
        ...
        @Override
        public void run() {
            if (StrUtil.equals(BlogConstant.OPTIONS.get(BlogPropertiesEnum.SMTP_EMAIL_ENABLE.getProp()), TrueFalseEnum.TRUE.getDesc()) && StrUtil.equals(BlogConstant.OPTIONS.get(BlogPropertiesEnum.NEW_COMMENT_NOTICE.getProp()), TrueFalseEnum.TRUE.getDesc())) {
                try {
                    // 发送邮件到博主
        ...
        public void run() {
            ...
            if (StrUtil.equals(BlogConstant.OPTIONS.get(BlogPropertiesEnum.SMTP_EMAIL_ENABLE.getProp()), TrueFalseEnum.TRUE.getDesc()) && StrUtil.equals(BlogConstant.OPTIONS.get(BlogPropertiesEnum.NEW_COMMENT_NOTICE.getProp()), TrueFalseEnum.TRUE.getDesc())) {
                if (Validator.isEmail(lastComment.getCommentAuthorEmail())) {
                    ...
                    mailService.sendTemplateMail(lastComment.getCommentAuthorEmail(), "您在" + BlogConstant.OPTIONS.get(BlogPropertiesEnum.BLOG_TITLE.getProp()) + "的评论有了新回复", map, "common/mail_template/mail_reply.ftl");
                }
            }
        }
    }
